...
class ArticulosController extends Controller
{
    ...
    {
        $articulo = Articulo::orderBy("updated_at", "desc")
            ->orderBy("created_at", "desc")
            ->with(["area", "fotos"])
            ->paginate(Config::get("constantes.paginas_en_paginacion"));

        $articulo->getCollection()->transform(function ($articulo) {
            $articulo->descripcion = Str::limit($articulo->descripcion, 30);
            return $articulo;
        });
    ...
    public function pdf()
    {
        ...
        $pdf = Pdf::loadView('articulos.pdf', compact('articulo'));
        return $pdf->stream();
    }
}
